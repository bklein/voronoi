...
int main(int argc, char** argv) {
...

  {
    ...
    for (int i=0; i<num_points; ++i) {
      points.emplace_back(std::make_pair(dis_x(gen), dis_y(gen)));
    }
  }
  {
    cv::Mat3b colormap;
  ...
  if (draw_centers) {
    for (const auto& pt : points) {
      const int x = static_cast<int>(std::round(pt.first));
      const int y = static_cast<int>(std::round(pt.second));
      cv::circle(image, cv::Point(x, y), (width / 200) + 1, cv::Scalar::all(0), -1);
    }
  }
